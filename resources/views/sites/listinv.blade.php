...
<div class = "main">
    <div class = "container_content">
        <div class = "left_part">
            <p class = "heading_le1">Запрошені експерти</p>
            <?php 
            $query = mysqli_query($connect, "SELECT * FROM list_experts WHERE permanent<1 ORDER BY name");
            while($rowe = mysqli_fetch_assoc($query)) {
                $page=App\List_expert::find($rowe['id']);     
            ?>
            <div class = "profile">
                <a href="permanent/{{$page->id}}">
                    <img class  = "profile_icon" src="{{Storage::url($page->photo)}}"></a>
                <div class = "profile_info">
                    <a href="permanent/{{$page->id}}"><p  class = "heading_le2">{{$page->name}}</p> </a>
                    <a href="permanent/{{$page->id}}"><p  class = "heading_le3">{{$page->prof}}</p></a>
                    <div class = "container_contact">
                        <img class  = "contact_icon" src="{{URL::asset('img/facebook_icon.png')}}">
                        <a  class = "heading_le3" href="{{$page->facebook}}">Facebook</a>
                    </div>
                    <div class = "container_contact">
                        <img class  = "contact_icon" src="{{URL::asset('img/mail_icon.png')}}">
                        <p  class = "heading_le3" href="">{{$page->email}}</p>
                    </div>
                    <!--<div class = "container_contact">
                        <img class  = "contact_icon" src="{{URL::asset('img/linkedin_icon.png')}}">
                        <a  class = "heading_le3" href="{{$page->linked}}">LinkedIn</a>
                    </div>-->
                </div>
            </div>
            <?php  } ?>
